<script>
  ...
  function calculatePages(response) {
    let total = Math.ceil(
      parseInt(escape(response.totalNumberOfResults)) /
        parseInt(escape(response.page.metadata.per))
    );
  ...
  function setAvatar(email) {
    let hash = md5(email);
    profileImage =
      "https://www.gravatar.com/avatar/" + hash + "?d=retro?s=300.jpg";
  }
...
<div class="main">
  <div class="drawer drawer-mobile">
    ...
    <div
      class="drawer-content flex flex-col items-center justify-center w-full"
    >
      <label
        for="main-drawer"
        style="z-index: 2"
      ...
      <div class="min-h-screen">
        <div
          class="grid 2xl:grid-cols-2 gap-x-8 gap-y-10 grid-cols-1 mb-10 mt-8"
        >
          {#await promise}
            <h1>Loading...</h1>
          {:then json}
                ...
                <div class="card-body md:w-6/12 w-full">
                  ...
                  <div class="card-actions justify-end">
                    <button
                      ...
                      on:click={() => goto("/snippet?" + escape(snippet.id))}
                      >Open</button
                    >
                  </div>
                </div>
    ...
    <div class="drawer-side w-full" style="width: 290px">
      ...
      <div class="px-4 bg-base-300">
          ...
          <div class="card-content">
            {#if isLoggedIn}
              ...
              {:then user}
                ...
                <button class="btn ml-5" on:click={() => goto("/account")}
                  >Settings</button
                >
                <button class="ml-3 mt-3 btn" on:click={() => goto("/create")}
                  >Create</button
                >
              {:catch error}
                <h1>{error}</h1>
        ...
        <div class="btn-group px-2 mt-10">
          <span class="btn" on:click={previousPage}
            ><h3 class="navbar-page-text">«</h3></span
          >

          {#each pages as page}
            {#if page == currentPage}
              <span class="btn active"
                ><h3 class="navbar-page-text">{page}</h3></span
              >
            {:else}
