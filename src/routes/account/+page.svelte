<script>
  ...
  async function login() {
    ...
    let encodedCredentials = window.btoa(
...
<div>
  {#if isLoggedIn}
    ...
    {:then user}
      {#if !isDeletingAccount}
        <div class="flex justify-center items-center">
          <button
            class="btn btn-error mr-3"
            on:click={() => (isDeletingAccount = true)}>Delete account</button
          >
      ...
      {:else}
        <div class="flex justify-center items-center">
          <div
            class="grid card bg-base-300 rounded-box place-items-center w-1/2"
          >
            ...
            <button
              class="btn mb-8"
              on:click={() => (isDeletingAccount = false)}>Cancel</button
            >
          </div>
        </div>
      {/if}
    {:catch error}
      <h1>{error}</h1>
      <p>
        Check whether credentials are correct. If error repeats, contact the
        developer.
      </p>
  ...
  {:else}
    <div
      class="grid md:grid-cols-2 grid-cols-1 gap-10 w-11/12 px-10 items-center justify-center"
    >
      ...
      <div class="grid card bg-base-300 rounded-box place-items-center">
        ...
        <button class="btn btn-primary mb-8" on:click={() => login()}
          >Login</button
        >
      </div>
    </div>
